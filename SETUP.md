...
2. Agent 会通过双段式工作流与你协作：
   - **第一部分 · 需求分析（必做）**
     ...
     - **阶段 2**：逐页面沟通详细规格 → 逐页确认
     - 产出并确认「需求包」（01-需求规格说明书.md + 02-UI交互规格说明书.md）
   - **决策门禁（需求包确认后触发）**：询问是否继续后续任务（当前仅「原型生成（Pencil 方案）」）
     - 拒绝 → 流程结束（prd-任务结果摘要.md 已随需求包定稿生成、prd-任务操作记录.md 已追加、产品工程摘要.md 已刷新）。
     - 同意 → 提示打开本地 Pencil 桌面应用并确保 Pencil MCP 服务已启动，进入第二部分
   - **第二部分 · 原型生成（可选）**：自动调用 Pencil 绘制 → 截图自校验 → 通知你验收
3. 所有产出文件（业务架构描述、页面规格、prd-任务结果摘要.md、prd-任务操作记录.md、产品工程摘要.md，以及可选的原型文件、截图、校验报告）都在 `document/{工程名}/{任务名}_{YYYYMMDD}/` 目录下（归属于对应工程项目）
...
```
# 在任务开始前或原型生成之前
将你自定义的 界面设计要素.md 放入 document/{工程名}/{任务名}_{YYYYMMDD}/
...
│   ├── prd-任务结果摘要.md                   # 产品任务结果摘要模板
│   ├── prd-任务操作记录.md                   # 产品任务操作记录模板
│   ├── 技术通用规则.md            # 技术通用规则模板（新建工程复制为 技术通用规则.md）
│   ├── 产品通用规则.md         # 产品通用规则模板
│   ├── 03-技术规格说明书.md            # 技术规格说明书模板（模块 → 功能点两层：模块归纳 + 逐功能点 9 子项含接口契约/伪代码）
│   ├── 04-数据模型说明书.md                   # 数据模型说明书模板（从 03 派生）
