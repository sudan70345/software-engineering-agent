...
> 可以直接使用此模板，也可以在任务目录中放入自定义的 `界面设计要素.md` 覆盖。
>
> **唯一权威源**：本文件是**视觉值**（颜色 / 字号 / 间距 / 圆角 / 组件尺寸 / 阴影）的唯一权威源。`系统交互规范.md` 与 `02-模块-UI交互规格.md` 只写**语义名**（如「主按钮」「标题字」），具体像素值一律引用本文件，不得在其内部重复硬编码色值或 px。

> **引用关系（与需求包联动）**：本文件是 `skills/page-specs/SKILL.md` 中「页面状态」「业务规则（产品行为）」章节的**视觉令牌来源**。
> - 02-模块-UI交互规格.md 中描述的页面状态（空 / 加载中 / 错误 / 受限）在视觉表达上应引用本文件的色板与组件令牌。
...
> 语义：`sm` = 平板竖屏起（≥768）；`md` = 桌面最小宽度（低于此宽度出现横向滚动）；`lg` = 桌面宽屏（≥1440 可用多列栅格）。
> 引用方：`02-模块-UI交互规格.md` §2 只写**端形态与基准视口区间档位**，`05-模块-前端技术规格.md` §7 写**断点适配落点**；两侧均**不写具体数值**。

## 布局与尺寸档位（语义 → 令牌）

> 供 `02-模块-UI交互规格.md` 引用：**区域 / 组件只写档位语义名，不写具体 px 与坐标**。

| 语义档位 | 令牌 | 说明 |
|---------|------|------|
| 顶栏高度（固定） | `navigation_bar_height` | 顶部导航栏固定高度 |
| 侧栏宽度（固定） | `side_nav_width` / `side_nav_width_collapsed` | 展开 / 收起 |
| 弹窗宽度档位 | `modal_width_small` / `modal_width_medium` / `modal_width_large` | 小 / 中 / 大 |
| 输入框高度档位 | `input_height` / `input_height_large` | 常规 / 大 |
| 按钮尺寸档位 | `button_small` / `button_medium` / `button_large` | 小 / 中（默认）/ 大 |
| 区域间距 / 内边距 | `spacing` 的 `element_gap` / `section_gap` / `page_margin` / `lg` / `xl` | 见「间距体系」 |
...
> 本节为 `02-模块-UI交互规格.md` 的「页面状态」「业务规则（产品行为）」提供**视觉令牌映射**，便于原型生成阶段（Pencil）与技术架构 Agent 对齐表现层。
...
> 映射原则：状态与反馈的**语义**由 02-模块-UI交互规格.md 定义，**视觉值**由本文件提供；原型生成阶段据此注入 Pencil 设计令牌，技术架构 Agent 可据此推导界面表现层约束。
